...
class TestModifierApplication(unittest.TestCase):
    ...
    def test_10_stim_primary(self):
        r = apply_modifier("mod_eg_av_add", intensity=3)
    ...
    def test_11_cleanse(self):
        ...
        apply_modifier("mod_eg_av_add", intensity=4)
        old_s = get_channel("e_g", "ch_g_s")
        r = apply_modifier("mod_eg_sv_shift", intensity=5)
    ...
    def test_12_doodle(self):
        r = apply_modifier("mod_eg_s_var", intensity=1)
        self.assertIn("ch_g_s", r.deltas)
        new_s = get_channel("e_g", "ch_g_s")
    ...
    def test_13_lws(self):
        r = apply_modifier("mod_eg_a_var", intensity=2)
        self.assertIn("ch_g_a", r.deltas)
    ...
    def test_16_lock_toggle(self):
        r = apply_modifier("mod_flag_02_toggle")
        self.assertEqual(r.effect_type, "flag_toggle")
        e = load_entity("e_g")
    ...
    def test_17_bound_strain(self):
        ...
        r = apply_modifier("mod_eg_av_add", intensity=1, strain_mult=2.0)
        a = get_channel("e_g", "ch_g_a")
    ...
    def test_18_lock_freezes_v(self):
        ...
        r = apply_modifier("mod_eg_av_add", intensity=5)
    ...
    def test_19_body_zone_numb(self):
        r = apply_modifier("mod_eb_zone_alter", zone="ch_b_03")
        self.assertEqual(r.effect_type, "state_set")
        self.assertEqual(get_channel("e_b", "ch_b_03"), 1)

    def test_20_body_zone_break(self):
        apply_modifier("mod_eb_zone_alter", zone="ch_b_05")
        r = apply_modifier("mod_eb_zone_break", zone="ch_b_05")
        self.assertEqual(get_channel("e_b", "ch_b_05"), 2)

    # ── 20. Body batch_restore ─────────────────────────────
    def test_21_batch_restore(self):
        apply_modifier("mod_eb_zone_alter", zone="ch_b_01")
        apply_modifier("mod_eb_zone_alter", zone="ch_b_02")
        apply_modifier("mod_eb_zone_alter", zone="ch_b_03")
        r = apply_modifier("mod_eb_restore")
    ...
    def test_25_stimulus_record(self):
        for _ in range(5):
            apply_modifier("mod_ex_count_add")
        self.assertEqual(get_channel("e_x", "ch_x_count"), 5)

    # ── 25. Batch apply ────────────────────────────────────
    def test_26_batch_apply(self):
        specs = [
            {"modifier": "mod_eg_av_add", "intensity": 2},
            {"modifier": "mod_eg_s_var", "intensity": 1},
            {"modifier": "mod_flag_01_toggle"},
        ]
        results = apply_modifiers_batch(specs)
        self.assertEqual(len(results), 3)
        self.assertEqual(results[0].modifier_id, "mod_eg_av_add")
        self.assertEqual(results[2].modifier_id, "mod_flag_01_toggle")
        a = get_channel("e_g", "ch_g_a")
