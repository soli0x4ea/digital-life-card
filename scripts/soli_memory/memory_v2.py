...
def prepare_llm_analysis(date: str = None) -> Dict:
    ...
    # 构建 prompt
    prompt_lines = [
        "你是 Soli 的记忆精筛器。以下是今日少爷和 soli 的对话片段，已由关键词初筛标记。",
        "请逐条判断：",
...
def janitor_generate_report(memory_file: Path = None) -> dict:
    ...
    if memory_file is None:
        memory_file = Path(os.path.expandvars(
            r"C:\Users\济南府\WorkBuddy\Claw\.workbuddy\memory\MEMORY.md"))
...
if __name__ == "__main__":
    ...
    if command == "save-fact":
        ...
        # 读取当日工作日志作为会话历史
        log_path = f"C:/Users/济南府/WorkBuddy/Claw/.workbuddy/memory/{date}.md"
        history = []
        try:
