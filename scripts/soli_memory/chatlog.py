...
用法：
  ...
  python chatlog.py log-user ...    # [已废弃] 手动记录少爷消息
  python chatlog.py log-assistant ..# [已废弃] 手动记录奴婢回复
"""
import sys
import json
import os
...
def cmd_status(today_only=False):
    ...
    if today_only:
        ...
        print(f"\n[chatlog] 今日记录：")
        print(f"  对话轮次：{convos} 轮（少爷 {user_count} 条 / 奴婢 {asst_count} 条）")
        if first_ts and last_ts:
...
def main():
    if len(sys.argv) < 2:
        ...
        print()
        print("已废弃命令（保留兼容）：")
        print("  log-user <内容>      手动记录少爷消息")
        print("  log-assistant <内容>  手动记录奴婢回复")
        sys.exit(1)
